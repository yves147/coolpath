...
def coords_to_distance(
    ...
    for el in coords_list[1:]:
        ...
        distance_to_last = math.sqrt(
            pow(last_coords[0] - el[0], 2) + pow(last_coords[1] - el[1], 2)
        )
        d += distance_to_last
...
def function_at_coords(
    ...
    return (
        temperature_alpha * temperature_at_coords(lat, long)
        + windspeed_beta * average_windspeed_at_coords(lat, long)
        + perceived_gamma * perceived_temperature_at_coords(lat, long)
        + climatope_zeta * climatope_at_coords(lat, long)
    )


def function_at_distance(
    distance,
    coords_list: list[tuple[float, tuple[float, float]]],
    ...
    for el in coords_list[1:]:
        ...
        if distance <= d:
            ...
            interp_x = (
                last_coords[0] + (current_coords[0] - last_coords[0]) * distance_ratio
            )
            interp_y = (
                last_coords[1] + (current_coords[1] - last_coords[1]) * distance_ratio
            )

            return function_at_coords(
                interp_x,
                interp_y,
...
def trapeziodal_integral(
    coords_list: list[tuple[float, float]] = list(),
    T: float = 0,
    n: int = 1024,
    perceived_gamma: float = 0.4,
    climatope_zeta: float = 2.0,
    ...
    if len(coords_list) == 2:
        T = math.sqrt(
            pow(coords_list[0][0] - coords_list[1][0], 2)
            + pow(coords_list[0][1] - coords_list[1][1], 2)
        )
        n = 2 * (int(T) + 1)
    ...
    dx = T / n
    sum = 0.5 * (
        function_at_distance(
            0,
            distance_coords_list,
            perceived_gamma=perceived_gamma,
            climatope_zeta=climatope_zeta,
        )
        + function_at_distance(
